#!/usr/bin/env python3
"""Plot the complete Run 9 latent-swap matrix for velocity-only fields."""
...
def reference_figure(dataset, rectangle, outdir):
    ...
    fig.suptitle("Run 9 reference CFD rectangle", y=0.99)
    save_figure(fig, outdir, "full_swap_reference_grid")


def swap_matrix_figure(result, outdir, errors=False):
    ...
    for channel_index, channel in enumerate(CHANNELS):
        ...
        fig.colorbar(image, ax=axes[channel_index, :].tolist(), shrink=0.82, pad=0.01)

    rectangle = result.rectangle
    kind = "absolute errors" if errors else "predictions"
    fig.suptitle(
        f"Run 9 full latent-swap {kind}: Re {rectangle.re_a:.0f} <-> "
...
def main(config_path, checkpoint_path, outdir):
    config = OmegaConf.load(config_path)
    if int(config.model.get("in_channels", 2)) != 2:
        raise ValueError("Run 9 full-swap figures require exactly two channels.")
    ...
    swap_matrix_figure(result, outdir, errors=False)
    swap_matrix_figure(result, outdir, errors=True)
...
if __name__ == "__main__":
    ...
    main(arguments.config, arguments.checkpoint, arguments.outdir)
