"""Full-swap model: Run 7 plus the missing latent combinations.
...
The full-swap objective adds AAB, ABA, BAB, and BBA on complete 2 x 2 (Re, geometry)
...
class FullSwapCompositionalAE(CompositionalAE):
    """Compositional autoencoder with the full-swap extension."""
    ...
    def full_swap_consistency(self, z_mu, z_g, z_xi, batch):
        """Train the four A/B assignments not covered by the base objective.
        ...
        * BBA -> B: residual-only swap into B.
        ...
        Reconstruction and cross-Re losses already cover AAA, BBB,
    ...
    def _losses(self, batch):
        """Upstream loss stack plus the full-swap term."""
